...
jQuery(document).ready(function ($) {
  Drupal.behaviors.copySymbol = {
  attach: function (context, settings) {
    
    $( ".clipboardjs-button" ).click(function() {
      var id = $(this).attr('data-clipboard-target');
      if(id){
        var val = $(id).val();
        $('#copied').text("Символ '" + val + "' скопирован в буфер обмена.");
      }
    });
...
jQuery(document).ready(function ($) {
 ...
 Drupal.behaviors.replaceReleaseNone = {

 attach: function (context, settings) {

   $('select#edit-field-release option:contains("- Select a value -")').text("- Выпуск -");
